...
def pose_to_matrix(rvec, tvec):
    """
    Converte una posa (rvec, tvec) in matrice di trasformazione omogenea 4x4.
    """
...
def offset_pose_to_center(T_pose, offset_xyz):
    """
    Applica una traslazione locale al frame originale per spostare l'origine.
    offset_xyz è un array (3,) in metri, es: [0.0375, 0.0375, 0.0]
    """
...
def matrix_to_pose(T):
    """
    Estrae rvec, tvec da una matrice di trasformazione 4x4
    """
    R_mat = T[:3, :3]
...
def rotation_matrix_to_quaternion(R_mat):
    """
    Converte una matrice di rotazione 3x3 in quaternione (qx, qy, qz, qw)
    """
